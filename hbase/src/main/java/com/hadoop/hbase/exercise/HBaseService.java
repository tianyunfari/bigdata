...
//		 hBaseService.getStatisticsByRowkey(tableName, row);
	...
	public void getStatisticsByRowkey(String tableName, String row) {
		// 获取表句柄
		Table table = myhbaseClient.getTable(tableName);
	...
	public void updateByIncrement(String row, String family, String column,
			long value) {
		// 获取表句柄hardcode
		Table table = myhbaseClient.getTable("statistics");
		try {
...
//				System.out.println(rowkey + ":" + taskid);
	...
	public void insertTask() {
		...
		for (Task task : list) {
			...
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_ACTUALFINISHTIME),
					Bytes.toBytes(task.getActualfinishtime()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_FINISHTIME),
					Bytes.toBytes(task.getFinishtime()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_NAME),
					Bytes.toBytes(task.getName()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_RECEIVETIME),
					Bytes.toBytes(task.getReceivedate()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_STARTTIME),
					Bytes.toBytes(task.getStarttime()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_STATE),
					Bytes.toBytes(task.getState()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_SYSTASKID),
					Bytes.toBytes(task.getSystaskid()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_TASKID),
					Bytes.toBytes(task.getTaskid()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_TYPE),
					Bytes.toBytes(task.getType()));
			put.addColumn(Bytes.toBytes(myhbaseClient.COLUMNFAMILY),
					Bytes.toBytes(myhbaseClient.COLUMNFAMILY_UID),
